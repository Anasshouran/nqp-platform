...
const SectorCmsNewsDetail = () => {
  ...
  usePageTitle('تفاصيل الخبر');

  useEffect(() => {
    if (!id || !sector) return;
    setLoading(true);
    Promise.all([getNewsById(id, sector.id), getNews(sector.id)])
      .then(([data, list]) => {
        setArticle(data);
  ...
  return (
    <SectorPageShell>
      <Button component={Link} to={`/sector/${slug}/news`} startIcon={<ArrowBackIcon />} sx={{ mb: 2 }}>
        العودة إلى أخبار القطاع
      </Button>
          ...
          <Grid item xs={12} md={8}>
            <Card sx={{ border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
              ...
              <Box sx={{ p: { xs: 3, md: 4 } }}>
                ...
                <Typography component="h1" variant="h4" sx={{ fontWeight: 700, mb: 2, lineHeight: 1.3 }}>
                  {article.title}
                </Typography>
                <Typography
                  component="div"
...
export default SectorCmsNewsDetail;