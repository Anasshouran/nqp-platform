...
class Command(BaseCommand):
    help = 'تهيئة ملف محلل الأحياء الدقيقة (محمد أحمد نور): الهيكل التنظيمي، الدور، الصلاحيات، البيانات الوظيفية'
    ...
    def handle(self, *args, **options):
        ...
        user = NqpUser.objects.filter(email='micro.analyst@nqp.sd').first()
        if not user:
            user = NqpUser.objects.create_user(
                email='micro.analyst@nqp.sd',
                full_name='Mohamed Ahmed Nur',
                password='testpass123',
                is_active=True,
            )
            self.stdout.write('تم إنشاء المستخدم micro.analyst@nqp.sd')
        ...
        position, created = OrgPosition.objects.get_or_create(
            code='MICRO_ANALYST_POSITION',
            defaults={
                'name_ar': 'محلل أحياء دقيقة',
                'name_en': 'Microbiology Analyst',
                'level': 9,
                'department': micro_section,
            },
        )
        if not created:
            position.name_ar = 'محلل أحياء دقيقة'
        ...
        role = Role.objects.filter(code='MICRO_ANALYST').first()
        if not role:
            self.stderr.write('دور MICRO_ANALYST غير موجود — شغّل seed_rbac أولاً')
            return
        ...
        profile.employee_number = 'LAB-MICRO-0011'
        profile.full_name_ar = 'محمد أحمد نور'
        profile.full_name_en = 'Mohamed Ahmed Nur'
        profile.job_title = 'محلل أحياء دقيقة'
        profile.gender = 'MALE'
        profile.employment_status = 'ACTIVE'
        profile.language = 'AR'
        profile.save()

        self.stdout.write(self.style.SUCCESS('اكتمل إعداد ملف محلل الأحياء الدقيقة بنجاح'))
